...
- **`withRetryOn429`** / **`withResultRetry`** (`src/core/withRetryOn429.ts`, 3.5.0) — функциональные retry-обёртки (`withReadRetry` удалён в 3.22.0 — та же функция под вторым именем).
...
- **Внутренние типы 3.5.0** для reliability-инфраструктуры: `KlineSubscriptionWatchdogArgs`, `KlineSubscriptionWatchdogConfig`, `KlineSubscriptionWatchdogDiagnostic`, `KlineWatchdogHealthEvent`, `KlineWatchdogStrategyArgs` (3.22.0), `RateLimitedRequestQueueArgs`, `WithRetryOn429Args`, `RateLimitConfig` (из `exchangeConnector.ts`). Общие типы сторожа — `StreamLastEntry`, `StreamOverdueEntry`, `StreamRecoveryState`, `StreamRecoveryAttemptResult`, `StreamRecoveryContext`, `StreamScanResultFormatArgs` (в 3.22.0 заменили клайновые дубли `KlineSubscriptionLastEntry`/`OverdueEntry`/`RecoveryState`; `WithReadRetryArgs` удалён).
