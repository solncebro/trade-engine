...
describeIfCredentials(
  ...
  'Bybit Error Handling Integration',
  () => {
    ...
    describe('Error Scenarios', () => {
      ...
      test('futures.setLeverage throws on invalid leverage', async () => {
        await expect(
          connector.futures.setLeverage(999, BYBIT_FUTURES_TEST_SYMBOL)
        ).rejects.toThrow();
      });

      test('futures.setMarginMode does not throw on valid input', async () => {
        await expect(
          connector.futures.setMarginMode(
      ...
      test('futures.fetchPosition throws on non-existent symbol', async () => {
        await expect(
          connector.futures.fetchPosition('NONEXISTENT_SYMBOL')
        ).rejects.toThrow();
      });
...
describeIfCredentials(
  ...
  'Binance Error Handling Integration',
  () => {
    let connector: ExchangeConnector;
